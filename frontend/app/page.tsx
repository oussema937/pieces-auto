...
// ── Design Tokens (skill: semantic color tokens, not raw hex in components) ──
const tokens = {
  bg: {
    base: '#0a0c0f',
    surface: '#111318',
...
// ── SVG Icons (skill: no emoji as structural icons) ──
const Icon = {
  Grid: () => (
...
function PieceModal({ piece, onClose, onDelete }: { piece: Piece; onClose: () => void; onDelete: (id: number) => void }) {
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Détails — ${get.nom(piece)}`}>
      <motion.div
        ...
        <div className="grid grid-cols-2">
          ...
          <div className="relative overflow-hidden" style={{ background: tokens.bg.base, minHeight: 280 }}>
            <img
              src={`http://localhost:8000/photo/${piece.id}`}
              alt={get.nom(piece)}
...
const [playing, setPlaying] = useState(false);
 ...
 useEffect(() => {
  ...
  // Démarre au premier scroll ou premier clic
  const start = () => {
    audio.play().then(() => {
      setPlaying(true);
    }).catch(() => {});
  ...
  const load = async () => {
    try {
      const r = await fetch('http://localhost:8000/pieces');
  ...
  const upload = async () => {
    ...
    const steps = [
      { p: 30, m: 'Transfert vers MinIO...' },
      { p: 60, m: 'Analyse IA en cours — LLaVA 13b...' },
    ...
    try {
      await fetch('http://localhost:8000/upload-multiple', { method: 'POST', body: fd });
      clearInterval(t); setScanPct(100); setScanMsg('Terminé.');
      await load();
      setTimeout(() => { setLoading(false); setFiles(null); setTab('catalogue'); }, 600);
    } catch { clearInterval(t); setLoading(false); alert('Erreur backend'); }
  };
  ...
  const del = async (id: number) => {
    ...
    setDeletingId(id);
    try {
      await fetch(`http://localhost:8000/pieces/${id}`, { method: 'DELETE' });
  ...
  // ── Hero section (shown before the app) ──
  if (!showApp) {
    return (
      <div style={{ background: '#0a0c0f' }}>
        <HeroScrub
          ...
          titleBottom="AUTO·IA"
          accentHex="#60a5fa"
          bgClassName="bg-[#0a0c0f]"
          badge="VISION IA — LLAVA 13B ACTIF"
        />

        {/* CTA section after the scroll sequence */}
        <div className="flex flex-col items-center justify-center py-32 px-6 text-center" style={{ background: '#0a0c0f' }}>
          ...
          <p className="text-sm max-w-md mb-10" style={{ color: tokens.text.secondary, lineHeight: 1.7 }}>
            Importez une photo, notre IA LLaVA 13b analyse et identifie automatiquement
            la pièce — marque, référence, état et prix estimé.
          </p>
          <motion.button
          ...
          <p className="text-xs mt-4" style={{ color: tokens.text.tertiary }}>
            LLaVA 13b · Ollama local · Stockage MinIO
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex flex-col" style={{ background: tokens.bg.base, color: tokens.text.primary, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <header className="fixed top-0 inset-x-0 z-40 h-14 flex items-center px-6 justify-between"
        ...
        <button onClick={() => setShowApp(false)} className="flex items-center gap-2.5" aria-label="Accueil">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: tokens.accent.blueDim }}>
            <Icon.Cpu />
          </div>
        ...
        <nav className="flex items-center gap-1" role="navigation" aria-label="Navigation principale">
          {navItems.map(({ id, label, icon: Ic }) => (
            <button key={id} onClick={() => setTab(id as typeof tab)}
              ...
              style={{
                color: tab === id ? tokens.text.accent : tokens.text.secondary,
                background: tab === id ? tokens.accent.blueDim : 'transparent',
                minHeight: 44,
              }}
              ...
              {id === 'catalogue' && pieces.length > 0 && (
                <span className="text-xs px-1.5 py-0.5 rounded-full" style={{ background: tokens.bg.elevated, color: tokens.text.tertiary }}>
                  {pieces.length}
                </span>
              )}
            </button>
  ...
  title={playing ? 'Pause' : 'Funk Sigilo'}>
  {playing ? (
    ...
    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            ...
            <button onClick={() => setShowNotif(!showNotif)}
              ...
              {notifs.length > 0 && (
                <span className="absolute top-1.5 right-1.5 w-3.5 h-3.5 rounded-full text-white flex items-center justify-center"
                  style={{ background: tokens.accent.red, fontSize: 9, fontWeight: 700 }}>
                  {notifs.length}
                </span>
              )}
            </button>
            <AnimatePresence>
              {showNotif && (
                <motion.div initial={{ opacity: 0, y: -8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.96 }}
                  transition={{ duration: 0.15 }}
                  className="absolute right-0 top-12 w-72 rounded-xl overflow-hidden shadow-2xl"
                  ...
                  <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: `1px solid ${tokens.border.subtle}` }}>
                    ...
                    {notifs.length > 0 && (
                      <button onClick={() => setNotifs([])} className="text-xs" style={{ color: tokens.text.tertiary }}>Effacer</button>
                    )}
                  </div>
                  {notifs.length === 0
                    ? <p className="px-4 py-6 text-xs text-center" style={{ color: tokens.text.tertiary }}>Aucune notification</p>
                    : notifs.map(n => (
                      <div key={n.id} className="px-4 py-3 text-xs" style={{ borderBottom: `1px solid ${tokens.border.subtle}`, color: tokens.text.secondary }}>{n.msg}</div>
                    ))}
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <main className="flex-1 pt-14">
        <AnimatePresence mode="wait">
          ...
          {/* Dashboard */}
         {tab === 'dashboard' && (
  <motion.div key="dash" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
    ...
    <motion.div className="flex items-end justify-between"
      ...
      <div>
        <motion.p className="text-xs font-medium mb-1.5 uppercase tracking-widest"
    ...
    <div className="grid grid-cols-3 gap-4">
      {[
        ...
        { label: 'Analysées par IA', value: analysed, accent: tokens.accent.green, sub: 'LLaVA 13b identifiées', icon: '✓' },
    ...
    <div className="grid grid-cols-3 gap-6">
      {/* Categories */}
      <motion.div className="col-span-2 rounded-xl p-5"
        style={{ background: tokens.bg.surface, border: `1px solid ${tokens.border.subtle}` }}
        initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.35, duration: 0.4, ease: 'easeOut' }}>
        <motion.p className="text-sm font-medium mb-5" style={{ color: tokens.text.primary }}
        ...
        <div className="space-y-3.5">
          {catCounts.map((c, i) => (
            <motion.div key={c.name}
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                <motion.span className="text-xs font-mono" style={{ color: tokens.text.tertiary }}
                  initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 + i * 0.06 }}>
                  {c.count}
                </motion.span>
              </div>
          ...
          {pieces.length === 0 && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
              className="text-xs text-center py-4" style={{ color: tokens.text.tertiary }}>
              Aucune donnée
            </motion.p>
          )}
        </div>
      ...
      {/* Recents */}
      <motion.div className="rounded-xl p-5 flex flex-col"
        style={{ background: tokens.bg.surface, border: `1px solid ${tokens.border.subtle}` }}
        initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.4, duration: 0.4, ease: 'easeOut' }}>
        <motion.p className="text-sm font-medium mb-4" style={{ color: tokens.text.primary }}
        ...
        <div className="flex-1 space-y-1">
          {pieces.slice(0, 5).map((p, i) => (
            <motion.button key={p.id} onClick={() => setSelected(p)}
              className="w-full flex items-center justify-between p-2.5 rounded-lg text-left group"
              style={{ minHeight: 44 }}
              initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }}
              ...
              <motion.span style={{ color: tokens.text.tertiary }} className="shrink-0 ml-2"
                whileHover={{ x: 2 }}>
                <Icon.ChevronRight />
              </motion.span>
            </motion.button>
          ...
          {pieces.length === 0 && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
              className="text-xs text-center py-6" style={{ color: tokens.text.tertiary }}>
              Aucune pièce
            </motion.p>
          )}
        </div>
        <motion.button onClick={() => setTab('catalogue')}
          ...
          {/* Scan */}
          {tab === 'scan' && (
            <motion.div key="scan" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              transition={{ duration: 0.2 }}
              ...
              <div>
                ...
                <p className="text-sm mt-1.5" style={{ color: tokens.text.secondary, lineHeight: 1.6 }}>
                  JPG, PNG, WEBP, BMP — sélection multiple. Chaque photo est analysée automatiquement par LLaVA 13b.
                </p>
              </div>
              ...
              {loading ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  ...
                  <div className="absolute top-0 inset-x-0 h-0.5" style={{ background: tokens.bg.overlay }}>
                    <motion.div animate={{ width: `${scanPct}%` }} transition={{ duration: 0.4, ease: 'easeOut' }}
                      className="h-full" style={{ background: tokens.accent.blue }} />
                  </div>
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <AnimatePresence mode="wait">
                      <motion.p key={scanMsg} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                        transition={{ duration: 0.2 }} className="text-sm" style={{ color: tokens.text.secondary }}>
                        {scanMsg}
                      </motion.p>
                    </AnimatePresence>
                  </div>
                ...
                <>
                  <motion.div
                    ...
                    style={{
                      background: dragging ? `rgba(96,165,250,0.06)` : tokens.bg.surface,
                      border: `1.5px dashed ${dragging ? tokens.accent.blue : tokens.border.default}`,
                      minHeight: 220,
                    }}
                    whileHover={{ borderColor: tokens.accent.blue }}
                    role="button"
                    tabIndex={0}
                    aria-label="Zone de dépôt de fichiers"
                    onKeyDown={e => { if (e.key === 'Enter') fileRef.current?.click(); }}>
                    <input ref={fileRef} type="file" multiple accept="image/*" onChange={e => setFiles(e.target.files)} className="hidden" aria-label="Sélectionner des fichiers" />
                    <div className="p-4 rounded-xl" style={{ background: tokens.bg.elevated }}>
                      <Icon.Upload />
                    </div>
                    <div>
          ...
          {/* Catalogue */}
          {tab === 'catalogue' && (
            <motion.div key="cat" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
              transition={{ duration: 0.2 }}
              ...
              <div className="flex gap-2 flex-wrap" role="group" aria-label="Filtrer par catégorie">
                {CATEGORIES.map(c => (
                  <button key={c} onClick={() => setFiltre(c)}
                    ...
                    style={{
                      background: filtre === c ? tokens.accent.blueDim : 'transparent',
                      color: filtre === c ? tokens.accent.blue : tokens.text.tertiary,
                      border: `1px solid ${filtre === c ? tokens.accent.blue + '40' : tokens.border.subtle}`,
                      minHeight: 32,
                    }}
                    aria-pressed={filtre === c}>
                    {c}
                    {c !== 'Toutes' && (
                      <span className="ml-1.5 font-mono" style={{ color: filtre === c ? tokens.accent.blue : tokens.text.tertiary, opacity: 0.7 }}>
                        {pieces.filter(p => get.cat(p) === c).length}
                      </span>
                    )}
                  </button>
                ))}
                      ...
                      return (
                        <motion.div key={p.id}
                          ...
                          <div className="relative overflow-hidden" style={{ height: 140, background: tokens.bg.elevated }}>
                            <img
                              src={`http://localhost:8000/photo/${p.id}`}
                              alt={get.nom(p)}
                          ...
                          <div className="p-3">
                            ...
                            <div className="flex items-center justify-between mt-1">
                              ...
                              {get.marque(p) !== '—' && (
                                <p className="text-xs truncate ml-2 shrink-0" style={{ color: tokens.text.tertiary }}>{get.marque(p)}</p>
                              )}
                            </div>
                          </div>
                        </motion.div>
